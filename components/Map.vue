<template>
  <section>
    <div id="addAddressMap" :class="mapClass"></div>
  </section>
</template>
...
load().then((mapglAPI) => {
  mapInstance.value = new mapglAPI.Map('addAddressMap', {
  ...
  if (!props.noControls) {
    const control = new mapglAPI.Control(
      mapInstance.value,
      `<button>
    ...
    control.getContainer().addEventListener('click', () => {
      if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition((position) => {
          ...
          const marker = new mapglAPI.Marker(mapInstance.value, {

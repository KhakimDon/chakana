<template>
  <div class="flex-y-center gap-2 justify-between rounded-xl px-4 py-3">
    ...
    <div class="w-24 shrink-0 text-right">
      ...
      >
        {{ formatMoneyDecimal(product?.discount_price || product?.price, 0) }}
        <span
...
const cartProducts = computed(() => cartStore.autoOrderProducts)
  ...
  if (count.value <= product?.max_quantity) {
    debounce(
      ...
      () => {
        ...
        orderCartStore
          .addToAutoOrderCart(product?.id, count.value, route.params.id)
          .then(() => {
            emit('refetch')
          })
          .catch(() => {
